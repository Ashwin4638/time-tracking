...
function App() {
  ...
  function handleTaskSubmit(e) {
    ...
    setTasks(prevTasks => [...prevTasks, { id: Date.now(), project, name, time, description, date }]);
    e.target.reset();
  }
      ...
      <div>
        ...
        <ul className='Projects'>
          {projects.map(project => (
            <li className='Project-List' key={project.id} onClick={() => handleProjectSelect(project.id)}>
              {project.name}
            </li>
          ))}
        </ul>
        {selectedProject && (
          <div>
            <h2>Selected Project: {projects.find(project => project.id === selectedProject).name}</h2>
